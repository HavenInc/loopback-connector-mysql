...
function mixinMigration(MySQL, mysql) {
  ...
  MySQL.prototype.alterTable = function(model, actualFields, actualIndexes, actualFks, done, checkOnly) {
    ...
    async.series([
      ...
      //ignore updatedAt & lastUpdated columns in find difference for auto update
      statements = statements.filter(s=> !(s.includes('CHANGE COLUMN `updatedAt`') || s.includes('CHANGE COLUMN `lastUpdated`')));
  ...
  function columnDefault(p, line) {
    ...
    if (p.hasOwnProperty('mysql') && p.mysql.hasOwnProperty('default')) {
      ...
      // TODO: add ON UPDATE CURRENT_TIMESTAMP support
    }
    return line;
  }
